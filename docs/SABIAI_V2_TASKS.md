...
**The V2.5.0 repository changes are frozen at the accepted production baseline `7acac8810bccb8773c20bb32be3f18bd833d6b3b`. The applicable runtime gates below passed, and the release was promoted through the Forgejo-first workflow on 2026-08-29.**
...
- Historical V2.4 evidence: GitHub `main` and tag `v2.4.0` resolved to the V2.4 baseline at `2ead1725b7b0eac4a95f05eef78bbbb5556cfdcd`. That baseline is superseded by the V2.5.0 production release below; the persistent `remote.github.pushurl=DISABLED` setting remains unchanged and no GitHub Actions were used.
- **Release status: PRODUCTION — Sabi Boy v2.5.0 / ENGINE COMPLETE.** The final evidence and current Forgejo/GitHub synchronization state are recorded in the V2.5 final production release section below.
...
### Phase 16 follow-up evidence — 2026-08-29 (V2.5 final production release)

- Production application commit: `7acac8810bccb8773c20bb32be3f18bd833d6b3b` on Forgejo `v2`, with the V2.5 engine-complete code and the settlement-heartbeat resilience fix. The previous application SHA `69fab7ed1b4ddb2f407135073b7da8a3f57dd0a4` was superseded before release; no force-reset or unknown local change was used.
- Full acceptance after the final fix passed on the Dell: **364 passed, 1 warning** (the existing Starlette/httpx deprecation warning). Branch, schema/migration reconciliation, OpenClaw gateway, read-only dashboard and backup/restore gates are all green. The exact report is `data/release/acceptance-v25-production-after-settlement-fix.json`.
- Migration/reconciliation remains exact: V1 closing bankroll **₦30,000**, migrated V1 bankroll **₦30,000**, `bankroll_matches=true`, `bets_matches=true`, `accumulators_matches=true` and `diary_matches=true`. Existing V2 history was preserved; the later V2 balance is not rewritten by migration.
- The production checkout now owns the original V2 port `127.0.0.1:8090`; the public `https://picks.hendrix.com.ng` route and local health both return HTTP 200 for Sabi Boy V2/read-only. `/api/v2/overview`, `/api/v2/notifications`, `/manifest.json` and `/sw.js` return successfully on both local and public paths. The V1 `sabiai-dashboard.service` is inactive; the tested V2.4 rollback checkout remains recoverable under `~/sabi-boy-migration-archives/v25-cutover-20260829T164349Z/v24-checkout`.
- OpenClaw activation and post-fix acceptance are green for technical agent `prediction` with human-facing identity Sabi Boy, the exact production workspace, seven current-format skills, the required V2.5 tool surface and readiness `READY`. The quiet daily/weekly reflections remain the only Sabi Boy OpenClaw automations; research, coverage, health, settlement and backup are system-owned.
- All five V2 timers are enabled: settlement (10 minutes), health (30 minutes), coverage radar (30 minutes), research (08:00 Africa/Lagos) and backup (daily). Manual health, coverage, settlement and backup runs completed successfully. The settlement run encountered one unavailable TheSportsDB lookup but now exits successfully with that issue retained as a structured warning, so one auxiliary provider cannot disable automatic settlement for other pending events. Regression coverage for this behavior is included in the 364-test result.
- The production research service also completed successfully under systemd using the configured Alibaba primary model path. The 2026-08-29 report considered 12 events and correctly recorded no qualifying recommendation when the configured Parse/TheSportsDB sources were rate-limited; no pick/ticket mutation or external wager occurred. The report remains at `data/reports/daily-picks-latest.json` with model usage and source failures preserved for review.
- The production source catalogue now contains **21** entries. API-Sports, SportsGameOdds, PandaScore, SportsDataIO and SportMonks are registered from private environment variables; optional providers that have not yet been called report `not_used_yet`, while known public/Parse sources report their observed healthy/degraded/down states. No source is represented as an unexplained `unknown` state after the source catalogue was refreshed.
- Live provider evidence is retained from the isolated Dell acceptance: API-Sports and PandaScore passed cache-aware probes; SportsGameOdds passed a filtered NBA event/odds request after its free-plan league/event requirement was respected; SportsDataIO (HTTP 404) and SportMonks (HTTP 403) are recorded as plan/access-limited optional sensors. Jolpica F1 and OpenLigaDB passed public probes. No Stake or 1xBet source or workflow is active.
- Backups: the final preproduction V1/V2 archive at `~/sabi-boy-migration-archives/v25-final-preproduction-20260829T164214Z/20260829T164214Z/manifest.json` verified checksums and integrity; the production backup heartbeat created a newer manifest under `data/backups/sabi-boy/` and the acceptance restore drill passed `PRAGMA quick_check=ok`.
- Security review: runtime environment, staging environment and VAPID private key are mode `600`; no credential values are tracked; the application binds loopback-only behind the existing route; the public response carries the restrictive CSP/security headers; no Cloudflare or production routing configuration was changed by this release. Existing documented limitations remain: public read-only data has no app login, inline styles are allowed by the CSP for the current shell, and optional external analytics remain blocked.
- Release status: **PRODUCTION — Sabi Boy v2.5.0 / ENGINE COMPLETE**. Coverage may still be degraded on days when free/optional providers rate-limit or restrict access; that is recorded as source coverage, not an engine-completeness failure. Forgejo remains the release source of truth; GitHub `main` promotion and the `v2.5.0` tag are the final repository synchronization step after this production evidence.
