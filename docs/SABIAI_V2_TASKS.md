...
- Migration/reconciliation is ready after the post-migration ledger fix: V1 closing balance **₦30,000**, migrated V1 ledger balance **₦30,000**, current V2 balance **₦27,812** after later V2 activity, post-migration delta **−₦2,188**, and `bankroll_matches=true`. The regression covers rerunning reconciliation after new V2 stakes without rewriting history.
- The post-stage coverage run is current and green: 131 canonical events, 18 priced events, 54 market-catalogue rows, 54 market offers, 21 prefiltered/researched events and 2 selected recommendations across a 72-hour horizon. The 182 source attempts produced 87 successes and 95 failures, predominantly TheSportsDB HTTP 429 responses; no paid or metered call was used by the coverage timer.
