...
class SabiToolGateway:
    ...
    def system_health(self, args: dict) -> dict:
        ...
        try:
            return {
                ...
                "database_ok": db.schema_version() is not None,
    ...
    def market_interpret(self, args: dict) -> dict:
        parsed = self.market_interpreter.interpret(
            str(args.get("text", "")), home=args.get("home"), away=args.get("away")
        )
        data = asdict(parsed)
    ...
    def _ticket_from_args(self, args: dict) -> Ticket:
        ...
        for index, raw in enumerate(args.get("legs", []), start=1):
            ...
            selection = Selection(
                market_id=market.id, label=parsed.plain_label, side=parsed.side
            )
            ticket.add_leg(
                TicketLeg(
