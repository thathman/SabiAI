...
class TicketWorkshop:
    def split(self, ticket: Ticket, slips: int) -> list[Ticket]:
        ...
        for index, legs in enumerate(buckets, start=1):
            children.append(Ticket(bookmaker_id=ticket.bookmaker_id,source_type="split",source_reference=ticket.source_reference,parent_ticket_id=ticket.id,status=ticket.status,legs=[replace(leg) for leg in legs],notes=[*ticket.notes, f"Split {index} of {slips} from ticket {ticket.id}."]))
        return children

    def trim_to_target(self, ticket: Ticket, target_odds: Decimal | float | str, *, min_legs: int = 1, max_states: int = 1500) -> Ticket:
        ...
        return Ticket(bookmaker_id=ticket.bookmaker_id,source_type="trim",source_reference=ticket.source_reference,parent_ticket_id=ticket.id,status=ticket.status,legs=[replace(leg) for leg in final_legs],notes=[*ticket.notes, f"Trimmed from {ticket.combined_odds} to {_product(final_legs)} toward target {target.quantize(Decimal('0.01'))}."])
    ...
    def remove(self, ticket: Ticket, leg_ids: set[str]) -> Ticket:
        ...
        return Ticket(bookmaker_id=ticket.bookmaker_id,source_type="edit",source_reference=ticket.source_reference,parent_ticket_id=ticket.id,status=ticket.status,legs=remaining,notes=[*ticket.notes, f"Removed {len(ticket.legs)-len(remaining)} leg(s)."])
